...
export const ReceivablesPage: React.FC = () => {
    ...
    const subRoutes = [
        { name: "Lease Model", link: "/receivables/lease" },
        { name: "Staff Commute Model", link: "/receivables/staff-commute" },
    ]
  
    useEffect(() => {
        if (location.pathname === "/receivables") {
            navigate("/receivables/lease")
        }
    },[location.pathname])
  
    return (
        <motion.div variants={pageVariants} initial='initial' animate='final' exit={pageVariants.initial} className="flex flex-col gap-3.5">
            <h1 className="text-grey-dark-1 font-bold text-2xl md:text-[2rem]">Receivables</h1>
