...
export const RiderWalletPage: React.FC = () => {
    ...
    const columns = [
      ...
      },
      {
        ...
        cell: ({ row }: { row: any; }) => {
          const item = row?.original as FetchedWalletTransaction
          return (
            <div className="text-sm text-grey-dark-2 capitalize whitespace-nowrap">{item?.status === 1 ? "Successful" : "Failed"}</div>
          )
        }
      }
    ];
  ...
  const trips = useMemo(() => {
      return [
          ...
          { label: "Inflow", value: formattedNumber(data?.credit_amount!), color: "bg-green-4" },
