...
export const PaymentValue: React.FC<PaymentValueDashboardProps> = ({ className, dues, paid }) => {
    const paidAmount = useMemo(() => {
        return [
            { label: "Lease Model", value:`${formattedNumber(paid?.total_lease_amount)} (${paid?.total_lease_count})` },
            { label: "Staff Plan Model", value: `${formattedNumber(paid?.total_commute_amount)} (${paid?.total_commute_count})` },
    ...
    const paidTotal = useMemo(() => {
        return [
            { label: "Total value of payments", value: formattedNumber(paid?.total_amount) },
    ...
    const duesAmount = useMemo(() => {
        return [
            { label: "Lease Model", value:`${formattedNumber(dues?.total_lease_amount)} (${dues?.total_lease_count})` },
            { label: "Staff Plan Model", value: `${formattedNumber(dues?.total_commute_amount)} (${dues?.total_commute_count})` },
            { label: "E-hailing Model", value: "₦0 (0)" },
        ]
    },[dues?.total_commute_amount, dues?.total_commute_count, dues?.total_lease_amount, dues?.total_lease_count])
    const duesTotal = useMemo(() => {
        return [
            { label: "Total value of payments", value: formattedNumber(dues?.total_amount) },
            { label: "Volume of payments", value: dues?.total_count },
        ]
